...
So, we will update the ‘ans’ variable with mid and search in the left half if there is any smaller index 
that satisfies the same condition. Here, we are eliminating the right half.
...
// int lb = lower_bound(vector.begin(), vector.end(), n) - vector.begin()
...
// int ub = upper_bound(vector.begin(), vector.end(), n) - vector.begin()
